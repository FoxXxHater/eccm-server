...
try {
    ...
    switch ($action) {
        ...
        case 'save_db':
            ...
            // Remove old db_config block, keep everything else (e.g. mail_config)
            $existing = preg_replace('/\$db_config\s*=\s*\[.*?\];\s*/s', '', $existing);
            // Ensure PHP tag
            if (strpos($existing, '<?php') === false) $existing = "<?php\n" . $existing;
