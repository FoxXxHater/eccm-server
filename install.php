...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $cfg  = "<?php\n";
        $cfg .= "/**\n * ECCM – Configuration\n * Auto-generated by installer / admin panel.\n */\n\n";
        $cfg .= "\$db_config = [\n";
        ...
        $cfg .= "    'token_lifetime'   => 3600,\n];\n\n";
        $cfg .= "// Override with local config if exists\n";
        $cfg .= "\$local_config = __DIR__ . '/config.local.php';\n";
        $cfg .= "if (file_exists(\$local_config)) {\n    include \$local_config;\n}\n";
        $written = @file_put_contents($cfgPath, $cfg);
        if ($written === false) {
            throw new Exception(
                'Konfigurationsdatei konnte nicht geschrieben werden: includes/config.php – ' .
                'Bitte Schreibrechte für den Ordner "includes/" setzen (chmod 755 oder 775).'
            );
        }
